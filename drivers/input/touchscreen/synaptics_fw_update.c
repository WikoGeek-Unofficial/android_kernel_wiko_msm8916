...
static int save_tp_info(char *product_id, char *config_id, int id, int rev_id)
{
	...
	else if(!strncmp(product_id, "DIJING", sizeof("DIJING"))){
            sprintf(buf, "DIJING-%s-S%d--V%d%d",
                    CONFIG_PRODUCT_NAME , id, config_id[2]-0x30, config_id[3]-0x30);        
        }
        else if(!strncmp(product_id, "YEJI", sizeof("YEJI"))){
...
#endif
//END<BUG><JABALL-0><20150831>store tp  info;xiongdajun
...
//Begin<BUG><JABALL-0><20150831>store tp  info;xiongdajun
	...
	printk("limi.zhang %s : %s %x.%x.%x.%x\n", __func__,fwu->product_id, fw_config_id[0], fw_config_id[1], fw_config_id[2], fw_config_id[3]);
